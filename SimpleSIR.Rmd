...
Deterministic and more simplistic stochastic SIR models for conspecific density vs. rec. rate combinations tested in spatially-explicit model
...
out <- ode(y = init, func = sir, times = times, parms = c(beta = 0.2, gamma = 0.1))
head(out)
...
out <- ode(y = init, func = sir, times = times, parms = c(beta = 0.2, gamma = 0.2))
head(out)
...
out <- ode(y = init, func = sir, times = times, parms = c(beta = 0.2, gamma = 0.4))
head(out)
...
##Box plots
Box plots comparing density and recovery rates
```{r}
...
##Box plot
Compare homogenous mixing and spatial models
```{r}
SIRdata_merged$percep<-"Homogenous"
SIRdata_merged$p<-NA
SIRdata_merged$H<-NA
...
h_vs_s <-rbind(sub_data, SIRdata_merged)
...
ii<- ii+ theme(axis.title = element_text(face="bold", size=16), title=element_text(face="bold", size=16))
ii<- ii + theme(axis.text.x = element_text(vjust=0.5, size=16))
...
setwd("C:/Users/law2y/OneDrive/R Code/LandscapeCh/Figures Dec. 2017")
tiff("Compare_homog_spatial.tif", height = 8, width = 14, units = "in", compression = "lzw", res = 300)
multiplot(hh,ii, cols=2)
dev.off()
```
