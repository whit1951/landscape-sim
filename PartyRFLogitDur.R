...
tic=Sys.time()
fit.cf1 <- cforest(duration ~ p + H+ beta1 +beta2 + beta3+ density + rec_rate + percep, data=merged_data, controls=cforest_unbiased(ntree=1000))
v1 <- varimp(fit.cf1, conditional= FALSE)
v1<-v1[order(v1)]
write.csv(v1, "partyRF_logitdur1000.csv")
